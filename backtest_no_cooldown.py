#!/usr/bin/env python3
"""
Markov Trading Bot Backtester - HYPER-AGGRESSIVE (NO COOLDOWN) STRATEGY
Simulates the bot's performance on 5-MINUTE historical data with ZERO fees.
"""
...
class MarkovBotBacktester:
    def __init__(self, start_date='2024-01-01', end_date='2024-12-31', 
        ...
        self.rebuild_frequency_periods = rebuild_frequency_hours * self.periods_per_hour
        self.matrix_training_window_periods = matrix_training_window * self.periods_per_hour
        ...
        print(f"🔬 Initializing NO-COOLDOWN backtest from {start_date} to {end_date}")
        ...
        print(f"💸 Trading Fee: {self.fee_pct*100:.2f}% per trade (ZERO FEE SIMULATION)")
        ...
        if self.enable_weekly_rebuild:
            print(f"   - Rebuild every {rebuild_frequency_hours} hours ({self.rebuild_frequency_periods} periods)")
            print(f"   - Training window: {matrix_training_window} hours ({self.matrix_training_window_periods} periods)")
    ...
    def fetch_historical_data(self, days=365):
        ...
        for exchange_name, symbol, max_limit in exchanges_to_try:
            try:
                ...
                periods_needed = days * 24 * self.periods_per_hour
                print(f"   Target: {periods_needed} periods ({days} days) of {self.timeframe} data...")
                
                all_candles = []
                batch_size = max_limit
                max_batches = min(40, (periods_needed // batch_size) + 2)  # Increased batches for 5m data
                ...
                for batch_num in range(max_batches):
                    try:
                        ...
                        if since_timestamp is None:
                            ...
                            candles = exchange.fetch_ohlcv(symbol, self.timeframe, limit=batch_size)
                        else:
                            # Subsequent batches - get older data
                            candles = exchange.fetch_ohlcv(symbol, self.timeframe, since=since_timestamp, limit=batch_size)
                        ...
                        since_timestamp = oldest_timestamp - (batch_size * 5 * 60000)  # Go back batch_size * 5 minutes
                        
                        # Check if we have enough data
                        if len(all_candles) >= periods_needed * 0.7:  # Accept 70% of target
                            print(f"   Reached acceptable amount of data: {len(all_candles)} periods!")
                            break
                ...
                if len(all_candles) >= 2000 * self.periods_per_hour:  # Check for reasonable amount of data
        ...
        actual_days = len(df) / (24 * self.periods_per_hour)
        print(f"✅ Successfully loaded {len(df)} periods ({actual_days:.1f} days) of data")
        print(f"   Period: {df.index[0]} to {df.index[-1]}")
        
        return df

    def calculate_indicators(self, df):
        """Calculate all technical indicators (same as main bot) adjusted for timeframe."""
        periods = self.periods_per_hour

        # RSI
        df['rsi'] = ta.momentum.RSIIndicator(df['close'], window=14 * periods).rsi()
        
        # Bollinger Bands
        bb_indicator = ta.volatility.BollingerBands(df['close'], window=20 * periods, window_dev=2)
        df['bb_upper'] = bb_indicator.bollinger_hband()
        df['bb_middle'] = bb_indicator.bollinger_mavg()
        df['bb_lower'] = bb_indicator.bollinger_lband()
        
        # EMA 200
        df['ema_200'] = ta.trend.EMAIndicator(df['close'], window=200 * periods).ema_indicator()
        
        # ATR
        df['atr'] = ta.volatility.AverageTrueRange(df['high'], df['low'], df['close'], window=14 * periods).average_true_range()
        df['atr_ma'] = df['atr'].rolling(window=20 * periods).mean()
        
        # Price change for Markov states (use 1-hour equivalent change)
    ...
    def build_transition_matrix(self, df, train_period=1000):
        ...
        min_train = 1000 * self.periods_per_hour
        max_train = available_data // 3
        
        actual_train_period = max(min_train, min(train_period * self.periods_per_hour, max_train))
        
        print(f"🧠 Building transition matrix from first {actual_train_period} periods...")
        print(f"   (Available data: {available_data} periods, using {actual_train_period/available_data*100:.1f}% for training)")
        
        train_data = df.iloc[:actual_train_period].dropna(subset=['markov_state'])
        matrix = build_transition_matrix(train_data)
        
        # Save for inspection
        with open('backtest_transition_matrix_no_cooldown.pkl', 'wb') as f:
    ...
    def rebuild_transition_matrix(self, df, current_idx):
        ...
        start_idx = max(0, current_idx - self.matrix_training_window_periods)
        end_idx = current_idx
        
        # Get training data from the window
        train_data = df.iloc[start_idx:end_idx].dropna(subset=['markov_state'])
        
        if len(train_data) < 100 * self.periods_per_hour:  # Ensure we have enough data
        ...
        filename = f'backtest_transition_matrix_no_cooldown_rebuild_{self.matrix_rebuild_count}.pkl'
    ...
    def generate_signal(self, current_idx, df, transition_matrix):
        ...
        # Technical analysis signals - CHAMPION LOGIC (RSI 30/70)
        ...
        elif (row['rsi'] > 70 or current_price > row['bb_upper']) and self.btc_holdings > 0.001:
            signal = 'SELL'
    ...
    def execute_trade(self, signal, price, timestamp, reason):
        ...
        if signal == 'BUY' and self.current_balance > 100:  # Only buy if we have cash
            ...
            # Fee simulation
            fee = btc_to_buy * self.fee_pct
            btc_received = btc_to_buy - fee

            # Execute the buy
            self.current_balance -= trade_amount
            self.btc_holdings += btc_received
            self.trade_count += 1
            ...
            position = {
                'type': 'long',
                'entry_price': price,
                'btc_amount': btc_received, # Track net amount after fees
            ...
            self.current_balance += cash_received
            self.trade_count += 1
    ...
    def run_backtest(self, days=400):
        ...
        for i in range(len(df)):
            ...
            if (self.enable_weekly_rebuild and 
                i > 0 and i - self.last_matrix_rebuild >= self.rebuild_frequency_periods):
    ...
    def analyze_results(self, df, initial_portfolio_value):
        ...
        periods_in_year = 365 * 24 * self.periods_per_hour
        sharpe_ratio = portfolio_df['period_return'].mean() / portfolio_df['period_return'].std() * np.sqrt(periods_in_year)
        ...
        # Print results
        print("\n" + "="*60)
        print("🎯 NO-COOLDOWN BACKTEST RESULTS")
        print("="*60)
        print(f"   Period: {portfolio_df['timestamp'].iloc[0]} to {portfolio_df['timestamp'].iloc[-1]}")
        print(f"⏱️  Duration: {len(portfolio_df)} periods ({len(portfolio_df)/(24*self.periods_per_hour):.1f} days)")
        ...
        if self.enable_weekly_rebuild:
            expected_rebuilds = len(portfolio_df) // self.rebuild_frequency_periods
            print(f"   Expected Rebuilds: {expected_rebuilds} (every {self.rebuild_frequency_periods // self.periods_per_hour // 24} days)")
    ...
    def plot_results(self, results):
        ...
        if len(trades_df) > 0:
            ...
            if len(buy_trades) > 0:
                ax1_twin.scatter(buy_trades['timestamp'], buy_trades['price'], 
                               color='green', marker='^', s=20, label='BUY', alpha=0.6)
            if len(sell_trades) > 0:
                ax1_twin.scatter(sell_trades['timestamp'], sell_trades['price'], 
                               color='red', marker='v', s=20, label='SELL', alpha=0.6)
        ...
        plt.tight_layout()
        plt.savefig('no_cooldown_backtest_results.png', dpi=300, bbox_inches='tight')
        plt.show()

def main():
    """Run the backtest for the specified period June 2024 - June 2025."""
    print("🔬 BACKTEST NO-COOLDOWN STRATEGY: June 2024 - June 2025")
    print("="*60)
    print("Testing Champion logic on 5-minute data with ZERO fees.")
    ...
    if results:
        print(f"\n🎯 BACKTEST NO-COOLDOWN BOT RESULTS!")
        ...
        if len(results['trades_df']) > 0:
            results['trades_df'].to_csv('backtest_no_cooldown_2024_2025_trades.csv', index=False)
        results['portfolio_df'].to_csv('backtest_no_cooldown_2024_2025_portfolio.csv', index=False)
        
        print(f"\n💾 Results saved to 'backtest_no_cooldown_2024_2025_...' files.")
